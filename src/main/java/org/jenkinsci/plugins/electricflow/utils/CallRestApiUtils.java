...
public class CallRestApiUtils {
    ...
    public static String perform(CallRestApiModel callRestApiModel, Run run, TaskListener taskListener)
            throws IOException {
        ...
        try {
            ...
            ElectricFlowClient efClient = ElectricFlowClientFactory.getElectricFlowClient(
                    callRestApiModel.getConfiguration(), callRestApiModel.getOverrideCredential(), run, envReplacer);
            ...
            String summaryHtml = getSummaryHtml(callRestApiModel, envReplacer, efClient, result);
            ...
            if (callRestApiModel.isEnvVarNameForResultSet()) {
                ...
                taskListener
                        .getLogger()
                        .println("Setting environment variable " + envVarForResult + "='" + result + "'");
    ...
    private static String getSummaryHtml(
            CallRestApiModel callRestApiModel, EnvReplacer envReplacer, ElectricFlowClient efClient, String result)
            throws IOException {
        ...
        String summaryText = "<h3>CloudBees CD Generic REST API</h3>"
                ...
                + "    <td>HTTP Method:</td>\n"
                + "    <td>"
                + HtmlUtils.encodeForHtml(callRestApiModel.getHttpMethod())
                + "</td>   \n"
                + "  </tr>\n";
        ...
        if (!HttpMethod.GET.equals(HttpMethod.valueOf(callRestApiModel.getHttpMethod()))) {
            ...
            if (!callRestApiModel.getParameters().isEmpty()) {
                ...
                summaryText = summaryText
                        ...
                        + "    <td>Body:</td>\n"
                        + "    <td>"
                        + HtmlUtils.encodeForHtml(formatJsonOutput(callRestApiModel.getBody()))
                        + "</td>    \n"
                        + "  </tr>\n";
            }
        }
        ...
        if (callRestApiModel.isEnvVarNameForResultSet()) {
            summaryText = summaryText
                    ...
                    + "    <td>"
                    + HtmlUtils.encodeForHtml(callRestApiModel.getEnvVarNameForResult())
                    + "</td>    \n"
                    + "  </tr>\n";
        }
        ...
        summaryText = summaryText
                ...
                + "    <td>Result:</td>\n"
                + "    <td><pre>"
                + HtmlUtils.encodeForHtml(formatJsonOutput(result))
                + "</pre></td>    \n"
                + "  </tr>\n";
