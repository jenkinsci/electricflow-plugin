...
public class BasicUnitTests {
  ...
  public void testSelectItemValidationWrapper() {
    ...
    assertTrue(
        Pattern.compile(".*\"validationMessage\":\"validation message\".*", Pattern.DOTALL)
            .matcher(selectItemValidationWrapperJsonStr)
            .matches());
    assertTrue(
        Pattern.compile(".*\"validationStatus\":\"OK\".*", Pattern.DOTALL)
            .matcher(selectItemValidationWrapperJsonStr)
            .matches());
    assertTrue(
        Pattern.compile(".*\"value\":\"value\".*", Pattern.DOTALL)
            .matcher(selectItemValidationWrapperJsonStr)
            .matches());
  }
